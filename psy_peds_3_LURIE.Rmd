...
# trying to remove kableExtra to run w/o docker
listOfPackages <- c("tidyverse", "RColorBrewer", "knitr", "tsModel", "gridExtra", "dplyr")
paket(listOfPackages)
```
...
```{r}
#setwd("~/projects/nu4ce/4ceData_files/Phase2.2.psy_peds_trends")
```
...
end_date_plots <- as.Date("2021-07-01")
```
...
admit_all_codes <- read.csv(paste0(folder_4ce_files, "/psych_admit_all_codes.csv"))
...
```{r}
nrow(unique(data.frame(admit_all_codes$patient_id)))
nrow(unique(data.frame(admit_demog$patient_id)))
```
...
Old codes that Nelson used. These were updated to the ICD code list above in order to remove codes for accidental poisoning

```{r}
icdCodes_old <- read.csv("public-data/psychiatric_ICD-10_codes_adjusted.csv", header = TRUE, colClasses = "character")

colnames(icdCodes_old) <- c("disorder_group", "ICD10_Code", "description")

icdCodes_old <- icdCodes_old %>% 
  mutate(ICD10_Code = gsub("ICD10:", "", ICD10_Code))

excluded_codes <- icdCodes_old %>% anti_join(icdCodes)
qc_check <- admit_all_codes %>% filter(icd_10_code %in% excluded_codes$ICD10_Code) 
nrow(unique(data.frame(qc_check$patient_id)))
```
...
```{r}
# admit_demog$pysch = 1
# non_psych_inpt_counts$pysch = 0

admit_demog <- rbind(admit_demog, non_psych_inpt_counts)

## appears to be no overlap?
#View(admit_demog %>% distinct(patient_id, encounter_id, pysch) %>% group_by(psych) %>%  mutate(count = n()))
```
...
# # filter ages
# admit_demog <- admit_demog %>% filter(age_years > 10 & age_years < 18)  %>%
#  filter(i_hsp_adm_dttm >= start_date_plots & i_hsp_adm_dttm <= end_date_plots)
...
nrow(admit_demog %>% distinct(patient_id, encounter_id))
...
encounters <- admit_demog %>% 
  ...
  mutate(encounter_group = seq_along(encounter_id),
         day_lag = as.numeric(round(difftime(i_hsp_adm_dttm, lag(i_hsp_dsch_dttm), unit = "hours"), 2))) %>% 
  data.frame() %>%
  ...
  consolidated = ifelse(day_lag <= 24, 1, 0))

# this is suggesting that 408 need to get consolidated?
table(encounters$consolidated)

check_lag <- encounters %>% filter(!day_lag == 100)
summary(check_lag$day_lag)
less24 <- check_lag %>% filter(day_lag <= 24) #449
summary(less24$day_lag)
hist(less24$day_lag)
...
encounters <- encounters %>%
  ...
  mutate(lag_date = lag(max_date),
         time_diff = as.numeric(round(difftime(i_hsp_adm_dttm, lag_date, unit = "hours"), 2))) %>%
  # adding 100 to temporarily handle the NAs
  mutate(time_diff = ifelse(is.na(time_diff), 100, time_diff),
    ...
    consolidated2 = ifelse(time_diff <= 24, 1, 0),
  # recalculate the encounter sequence
  encounter_num = dense_rank(i_encounter_group)) %>% 
  group_by(patient_id, encounter_num) 

# this is suggesting that 408 need to get consolidated?
table(encounters$consolidated2)

check_lag <- encounters %>% filter(!time_diff == 100)
summary(check_lag$time_diff)
less24 <- check_lag %>% filter(time_diff <= 24) #397 patients/admissions said to need to be consolidated
summary(less24$time_diff)
hist(less24$time_diff)

encounters <- encounters %>% 
  slice(1L)

nrow(encounters %>% distinct(patient_id, encounter_num))
```
...
nrow(clinical_raw %>% distinct(patient_num, encounter_num))
```
...
codesToReview <- admit_all_codes %>% dplyr::filter(icd_10_code == "DIAG-ICD10",
                                           ! grepl( "[A-Z][0-9][0-9AB]\\.?[0-9A-TV-Z]{0,4}", icd_10_code))
print(codesToReview)
...
mentalHealthCodes <- unique( admit_all_codes %>%
  dplyr::filter( #concept_type == "DIAG-ICD10", 
                 icd_10_code %in% icdCodes$ICD10_Code ) %>%
  dplyr::mutate( icd_10_code = as.character( icd_10_code ), 
                 concept_length = nchar( icd_10_code ), 
                 concept_check = ifelse( icd_10_code == 4 | concept_length > 8, "Check", "OK")) %>%
  dplyr::select( icd_10_code, concept_length, concept_check) )

summary(as.factor( mentalHealthCodes$concept_check ) )

print( paste0("There are ", length(mentalHealthCodes$icd_10_code), " ICD10 mental codes out of the total ",length(unique(icdCodes$ICD10_Code)), " in the dataset"))
```
...
date_ids <- seq(as.Date("2019-01-01"), as.Date(end_date_plots), 
                by= "day") %>% 
  data.frame() %>% 
...
nrow(clinical_filtered %>% distinct(patient_num, encounter_num))
...
nrow(input_data %>% distinct(patient_num, encounter_num))
```
...
qc_check <- admit_all_codes %>% 
  ...
  dplyr::filter( diagnosis_code_source == "ICD-10-CM", 
                 ...
                 age > 10 & age < 18,
                 date >= start_date_plots & date <= end_date_plots,
                 ) %>%
...
Are there any patients in the admit_demog df who do not have any icd codes listed?
  
Note: this looks fine
```{r}
admit_demog <- read.csv(paste0(folder_4ce_files, "/psych_admit_demog.csv"))

# list of all patients who should have had at least 1 psych diagnosis
psych_ids <- admit_demog %>% 
  distinct(patient_id)
nrow(psych_ids)

# evaluate whether there are patients in admit_all_codes file that were not listed in the admit_demog file
admit_demog_ids <- admit_all_codes %>% 
  distinct(patient_id) %>% 
  filter(!patient_id %in% psych_ids$patient_id)
nrow(admit_demog_ids)

# evaluate whether there are patients in the admit_demog file that do not have diagnosis codes
psych_id_check <- psych_ids %>% 
  distinct(patient_id) %>% 
  filter(!patient_id %in% admit_all_codes$patient_id)
nrow(psych_id_check)
```
...
input_data <- input_data %>% 
  ...
  dplyr::filter(date >= start_date_plots & date <= end_date_plots)
...
# keep non_psych whose dx date did not occur during the admission
# but correct the diagnosis data to match the non_psych group
keep_non_psych_v2 = input_data %>% 
  group_by(patient_num, encounter_num) %>% 
  filter(i_diagnosis_dttm < date | i_diagnosis_dttm >= i_hsp_dsch_dttm) %>% 
  dplyr::select( patient_num, encounter_num, sex, age, race, concept_code, n_hospitalisation, len_hospitalisation, 
                 date, weeks, month, year, period, time_p, disorder_group, description, icd_code_category) %>% 
  mutate(concept_code = NA,
         disorder_group = NA,
         description = NA, 
         icd_code_category = NA)

input_data <- rbind(keep_psych, keep_non_psych, keep_non_psych_v2)
...
nrow(all_codes_desc %>% distinct(patient_num, encounter_num))

all_codes_psy <- all_codes_desc %>% 
  filter(concept_code %in% icdCodes$ICD10_Code)

nrow(unique(data.frame(all_codes_psy$patient_num)))
nrow(all_codes_psy %>% distinct(patient_num, encounter_num))
# View(all_codes_psy %>% 
#        ungroup() %>% 
#        distinct(patient_num, encounter_num, time_p, period) %>% 
#        group_by(period) %>% 
#        mutate(count = n_distinct(patient_num, encounter_num)) %>% 
#        distinct(period, count))

## how many weeks in 'during' vs 'before' pandemic (15 for both)
# View(all_codes_psy %>% 
#        ungroup() %>% 
#        distinct(month, period) %>%
#        group_by(period) %>% 
...
#unique patient and encounter by month
test <- all_codes_desc %>%
               #filter(icd_code_category == "psy") %>% 
               # first, for each month, we will modify the data, so that if a patient has a psych AND non-psych admission in the first month, we will only count their psych admission
               group_by(month) %>% 
...
mental_codes_qc <- all_codes_desc %>% 
  filter( concept_code %in% mentalHealthCodes$icd_10_code)
...
First calculation: total number of patients, matching the inclusion criteria, admitted on one day (without filtering by any disease subtype). Summed up over one week, or one month depending on the time unit chosen. Not filtered on age. 
...
length_hospitalisation_values <- all_codes_psy %>% 
  dplyr::distinct( patient_num, encounter_num, n_hospitalisation, len_hospitalisation, period ) %>%
  dplyr::mutate( siteid = site ) %>%
  dplyr::select( len_hospitalisation, siteid, period )

# de-identify to share with alba/arnaud
to_share <- length_hospitalisation_values %>% 
  ungroup() %>% 
  mutate(siteid = "Lurie") %>% 
  select(len_hospitalisation, siteid, period)

write.csv(to_share, "../2022.04.06_Lurie_Output/Lurie_los.csv", row.names = FALSE)
...
save( count_icd,
      ...
      file = paste0("../2022.04.06_Lurie_Output/peds_psy_aggResults_", site, ".RData"))
