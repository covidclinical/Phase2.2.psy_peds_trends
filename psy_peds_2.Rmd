...
new_hospitalizations_per_month3 <- new_hospitalizations_per_day %>%
  group_by( month) %>%
  summarise(count = sum(count))
...
total_hospitalizations_per_week <- total_hospitalizations_per_day %>% 
  filter(weeks > '2019-03-01') %>% 
    group_by( weeks ) %>%
  summarise(count = sum(count))

total_hospitalizations_per_month <- total_hospitalizations_per_day %>%
  group_by( month ) %>%
  summarise(count = sum(count))
