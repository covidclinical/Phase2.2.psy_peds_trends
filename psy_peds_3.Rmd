...
```{r}
unique_patients_hospitalized_per_week <- clinical_table_ado %>%
  filter(date < end_date_plots &
         date >= start_date_plots ) %>%
    group_by( weeks ) %>%
  summarise(count = n_distinct(patient_num))

th_week <- admission_plot_count(df = unique_patients_hospitalized_per_week, 
                     x = "weeks",
                     y = "count", 
                     title = "Total hospitalization (per week)", 
                     y_title = "Patient Count") +
...
##### Total hospitalizations Per week
