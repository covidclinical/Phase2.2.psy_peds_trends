...
listOfPackages <- c("tidyverse", "RColorBrewer", "knitr", "kableExtra", "tsModel", "gridExtra")
...
observation_table <- obs_filtered %>% 
  inner_join(distinct(clinical_table, patient_num, days_since_admission), by = c("patient_num", "days_since_admission"))
demographic_table <- demo_raw %>% 
  inner_join(distinct(clinical_table, patient_num), by = "patient_num")
all_codes <- observation_table %>% 
  inner_join(clinical_table, on = c("patient_num", "days_since_admission")) %>% 
  inner_join(select(demographic_table, -days_since_admission), on = c("patient_num"))
...
new_hospitalizations_per_day <- clinical_filtered %>%
...
nh_day <- admission_plot_count(df = new_hospitalizations_per_day, 
                     x = "date",
                     y = "count", 
...
nh_month <- admission_plot_count(df = new_hospitalizations_per_month, 
                     x = "month",
                     y = "count", 
...
count_icd_per_day$percentage_psy <- count_icd_per_day$count_psy / count_icd_per_day$count_icd
