...
all_codes <- obs_raw %>% 
  ...
  dplyr::select( patient_num, cohort, sex, age, concept_type, concept_code, calendar_date,
                 in_hospital, n_hospitalisation, len_hospitalisation, date, weeks, year,
                 period, disorder_group, description )
...
patient_count_barplot_period$orderDisorder <- factor(patient_count_barplot_period$disorder_group,levels = as.factor( order_disorders$disorder_group ))
...
```{r}
patient_count_per_week <- input_data[, c("patient_num", "cohort", "weeks")] %>%
  unique() %>%
  group_by(weeks) %>%
  tally() %>%
...
patient_count_per_week %>%
  ...
  dplyr::mutate(period = ifelse( weeks < pandemic_start_date,
                                  "before_pandemic",
                                 "during_pandemic")) %>% 
  ggplot(aes(x = weeks, y = n, fill = period)) +
