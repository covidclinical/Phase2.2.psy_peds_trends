...
>>>>>>> e33bae663cc501bd16ff3a77ae7430f0c3299cc3
```
...
## Overview of daily hospitalizations in the site

```{r}
# using the patient level data
all_bch_patients <- obs_raw %>% 
  full_join( clinical_raw,  by = c("patient_num", "cohort", "days_since_admission")) %>%
  full_join( demo_raw, by = c("patient_num", "cohort")) %>% 
  dplyr::filter( cohort == "AllAdm",
                 in_hospital == 1,
...
patient_count_per_day <- all_bch_patients[, c("patient_num", "calendar_date")] %>%
  mutate( calendar_date = as.Date(calendar_date, format=dateFormat)) %>%
  unique() %>%
  group_by(calendar_date) %>%
  tally( )

patient_count_per_day %>%
  ggplot(aes(x = calendar_date, y = n)) +
  geom_point()+
  labs(y = "Counts",
       x = "Days",
       title = "Hospitalized patients by day") 

### using the aggregate counts (1.2 data)
daily_agg <- read.csv("/4ceData/Input/DailyCounts.csv")

daily_agg %>%
  filter( cohort == "AllAdm") %>%
  ggplot(aes(x = calendar_date, y = pts_in_hosp_on_this_date)) +
  geom_point()+
  #scale_fill_manual() +
  labs(y = "Counts",
       x = "Days",
...
encounters_obs <- clinical_raw %>% 
  ...
  mutate(psychiatric_code = ifelse(concept_code %in% icdCodes$ICD10_Code, TRUE, FALSE)) %>% 
  group_by(patient_num, n_hospitalisation) %>% 
  summarise(psy_encounters = any(psychiatric_code)) %>% 
  mutate(psy_encounters = ifelse(isTRUE(psy_encounters), "is_psy_encounter", "not_psy_encounter"))
...
obs_by_day <- encounters_obs %>% 
  ...
  group_by(calendar_date, psy_encounters) %>% 
  ...
  pivot_wider(names_from = psy_encounters, 
              values_from = patient_count) %>% 
  mutate(percentage = is_psy_encounter / (is_psy_encounter + not_psy_encounter)) %>% 
  filter(calendar_date <= as.Date("2021-06-01"))
...
ggplot(obs_by_day, aes(x = calendar_date,
  ...
  #           linetype = "dashed") +
  xlim(as.Date("2019-01-01"), as.Date("2021-05-01")) +
  ylim(0, 0.4) +
  scale_color_manual(values = cbPalette) +
