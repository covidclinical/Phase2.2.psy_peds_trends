...
listOfPackages <- c("tidyverse", "RColorBrewer", "knitr", "kableExtra")
...
folder_4ce_files <- "/4ceData/Input"
obfuscation =  FALSE
#obfuscation = 3
raceAvailable = TRUE
#raceAvailable = FALSE
...
### Estimate how many of the mental codes are in the dataset
...
clinical_raw <- clinical_raw %>% 
  ...
  dplyr::filter(cohort  %in% c("AllAdm", "AllNotAdm"))
```
...
```{r}
all_codes[, c("patient_num", "cohort", "month")] 
```


```{r}
patient_count_per_week <- all_codes[, c("patient_num", "cohort", "weeks", "icd_code_category")] %>%
  unique() %>%
  group_by(weeks, icd_code_category) %>%
  tally() %>%
...
patient_count_per_month <- all_codes[, c("patient_num", "cohort", "month", "icd_code_category")] %>%
  unique() %>%
  group_by(month, icd_code_category) %>%
  tally() %>%
  mutate(n = ifelse( n < obfuscation | isFALSE(obfuscation),
                     n,
                     0.5),
...
patient_count_per_week %>%
  tidyr::pivot_wider(names_from = icd_code_category, 
                     values_from = n) %>% 
  dplyr::mutate(weeks = as.Date(weeks)) %>% 
  dplyr::mutate(period = ifelse( weeks < pandemic_start_date,
                                  "before_pandemic",
                                 "during_pandemic"), 
                percentage = psy * 100 / (somatic + psy)) %>% 
  ggplot(aes(x = weeks, y = percentage, fill = period)) +
...
patient_count_per_week %>%
  ...
  dplyr::mutate(period = ifelse( weeks < pandemic_start_date,
                                  "before_pandemic",
                                 "during_pandemic")) %>% 
  ggplot(aes(x = weeks, y = n, fill = icd_code_category)) +
  # geom_bar(stat="identity")+
  scale_fill_manual(values = cbPalette) +
  labs(y = "Counts",
       x = "Calendar Weeks",
       title = "Weekly patient counts with mental health related ICD codes") + 
  geom_smooth(method = "lm", formula = y ~ splines::ns(x, 2), se = TRUE, aes(colour=icd_code_category), colour= "black", size = 0.5)
...
```{r}
patient_count_per_week <- all_codes %>% 
  select(patient_num, cohort, weeks, icd_code_category, period) %>% 
  unique() %>%
  group_by(weeks, icd_code_category, period) %>%
  tally() %>%
  mutate(n = ifelse( n < obfuscation | isFALSE(obfuscation),
                     n,
                     0.5),
...
#estimate the number of patients per week
patient_count_per_week %>%
  ggplot(aes(x = weeks, y = n, fill = icd_code_category, color = icd_code_category)) +
  geom_point() +
  geom_line() +
...
colnames(icdCategory) <- c("icd_code_category", "icd_description_category")
patient_count_per_week_byCategory <- all_codes %>% 
  mutate( icd_code_category = ifelse(substr(concept_code, 1, 1) %in% c("D", "H"), substr(concept_code, 1, 2), substr(concept_code, 1, 1) ))

patient_count_per_week_byCategory <- left_join( patient_count_per_week_byCategory, icdCategory) %>%
  mutate( icd_description_category = ifelse( concept_code %in% icdCodes$ICD10_Code, "psy", icd_description_category)) %>%
  select(patient_num, cohort, weeks, icd_description_category, period) %>% 
  unique() %>%
  group_by(weeks, icd_description_category, period) %>%
  tally() %>%
  mutate(n = ifelse( n < obfuscation | isFALSE(obfuscation),
                     n,
                     0.5),
...
patient_count_per_week_byCategory %>%
  filter( icd_description_category %in% c("psy","Neoplams","Diseases of the respiratory system", "Diseases of the nervous system", "Diseases of the digestive system", "Diseases of the blood", "Congenital malformations, deformations and chromosomal abnormalities" )) %>%
  ggplot(aes(x = weeks, y = n, fill = icd_description_category, color = icd_description_category)) +
  ...
  ggsave("output/overall_weekly_count_all_comparison_other_categories.png", width = 18, height = 9, units="cm")

patient_count_per_week_byCategory %>%
  filter( icd_description_category %in% c("Neoplams","Diseases of the respiratory system", "Diseases of the nervous system", "Diseases of the digestive system", "Diseases of the blood", "Congenital malformations, deformations and chromosomal abnormalities" )) %>%
  ggplot(aes(weeks, n, color = factor(icd_description_category))) +
  geom_line() +
  geom_vline(xintercept = as.Date(pandemic_start_date),
             linetype = "dashed") +
  geom_point() + 
  theme(axis.text.x = element_text(angle=45, vjust = 1, hjust=1), legend.position = "none" ) +
  labs(title = "Weekly patient counts",
       subtitle = "(by ICD category)",
       y = "Counts", x = "Calendar weeks") + 
  facet_wrap(~ icd_description_category)

ggsave("output/overall_weekly_count_all_comparison_other_categories_facet_wrap.png", width = 18, height = 9, units="cm")
...
patient_count_per_week_disorder_group <- input_data[input_data$in_hospital == 1, c("patient_num", "cohort", "weeks", "disorder_group")] %>%
  unique() %>%
  group_by(weeks, disorder_group) %>% tally() %>%
  mutate(n = ifelse( n > obfuscation | isFALSE(obfuscation), n, 0.5), siteid = site)
...
patient_count_per_week_disorder_group %>%
  ...
  ggplot(aes(x = as.Date(weeks), y= n, fill = period, group = disorder_group )) +
  geom_bar(stat = "identity")+
  geom_vline(xintercept = as.Date(pandemic_start_date),
             linetype = "dashed") +
  #geom_vline(xintercept = as.Date(late_pandemic_start_date),
