...
new_hospitalizations_per_day <- clinical_raw %>%
group_by(patient_num, n_hospitalisation) %>% slice(which.min(as.Date(calendar_date, dateFormat))) 

newhosp <- new_hospitalizations_per_day %>%
    filter( in_hospital == 1, 
          as.Date( calendar_date, dateFormat ) < '2021-05-01' &
           as.Date( calendar_date, dateFormat ) > '2019-03-01' ) %>%
    group_by( calendar_date) %>%
  summarise(count = n_distinct(patient_num))

ggplot(newhosp, aes(x = as.Date(calendar_date, dateFormat),
                       y = count))  +
  geom_point() +
  ...
  labs(x = "Calendar date",
       y = "Patients count", 
       title = "New daily admissions (2.2 data)")


xx <- clinical_raw[ , .SD[which.min(as.Date(calendar_date, dateFormat))], by = c(n_hospitalization, patient_num)]
...
all_codes <- obs_raw %>% 
  ...
  dplyr::filter( concept_type == "DIAG-ICD10", 
                 in_hospital == 1,
                 age_time_visit < 18 & age_time_visit > 10,
                 date >= history_start_date & date <= history_end_date) %>%
  dplyr::select( patient_num, cohort, sex, age, age_time_visit, concept_type, concept_code, calendar_date,
...
ts_seasonal_percent <- lm(percentage ~ period*time + harmonic(month_year, 1, 12), patient_count_monthly_psy)
...
input_data[, c("patient_num", "cohort", "age_time_visit","weeks")] %>%
  unique() %>%
  mutate( age_group = ifelse( age_time_visit < 15, "early adolescence (11-14 years)", "middle adolescence (15-17 years)")) %>%
...
age_stats <- input_data %>%
  ...
  dplyr::filter(age_time_visit == max(age_time_visit)) %>%
  dplyr::ungroup() %>% 
  dplyr::distinct(patient_num, cohort, period, age_time_visit) %>% 
  dplyr::filter(age_time_visit != 0) %>% 
  dplyr::group_by( period ) %>% 
  dplyr::summarise( mean = mean( age_time_visit ), 
                    sd = sd( age_time_visit ), 
                    count = n()) %>% 
  dplyr::mutate(count = ifelse( count > obfuscation, count, obfuscation * 0.5)) %>%
  dplyr::ungroup() %>% 
  dplyr::bind_rows(
    dplyr::summarise(input_data,
                     period="Total",
                     mean = mean(age_time_visit),
                     sd = sd(age_time_visit),
                     count = n())
  )
