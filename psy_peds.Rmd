...
count_sequences_hospitalisation <- function(df, ...) {
  # TODO: To be optimized to make it faster 
  total_span <- data.frame(total_span = seq(min(df$days_since_admission),
                    max(df$days_since_admission))
  )
  total_span <- left_join(total_span, df[,c("in_hospital", "days_since_admission")],
            by = c("total_span" = "days_since_admission")
  ) %>% replace_na(list(in_hospital = 0))
  count_sequences <- rle(total_span$in_hospital)
...
patient_count_monthly %>%
  dplyr::filter(icd_description_category == "Psy") %>% 
