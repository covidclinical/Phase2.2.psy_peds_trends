...
We will also transform the observation dates, and break it by weeks, adding 2 additional columns: "weeks" and "year". Finally, we will dichotomize the study timeframe in two periods, based on cut-off dates, "before_pandemic", and "after_pandemic". 
...
# cut-off dates used in the analysis 
...
input_data <- obs_raw %>% 
  ...
  # dplyr::mutate(period = ifelse( date < early_pandemic_start_date,
  #                                 "before_pandemic",
  #                                 ifelse(date < late_pandemic_start_date,
  #                                        "early_pandemic", 
  #                                        "late_pandemic"))) %>%
  dplyr::mutate(period = ifelse( date < pandemic_start_date,
                                  "before_pandemic","after_pandemic")) %>%
...
- both before and after pandemic 
...
patient_count_barplot_period <- input_data %>% 
  dplyr::group_by( disorder_group, patient_num ) %>% 
  dplyr::mutate( 
    when = ifelse( "before_pandemic" %in% period & "after_pandemic" %in% period,
                   "Before and After Pandemic",
                   ifelse( "before_pandemic" %in% period & ! "after_pandemic" %in% period,
                           "Only Before Pandemic",
                           "Only After Pandemic"
                   )
    ) 
    %>% factor(levels = c("Only Before Pandemic", "Before and After Pandemic", "Only After Pandemic"))
  ) %>%
  dplyr::ungroup() %>% 
  dplyr::select( patient_num, disorder_group, when ) %>%
...
ggsave("output/disorder_before_after_pandemic_all.png", width = 18, height = 9, units="cm")
```
...
```{r}
patient_count_per_week <- input_data[, c("patient_num", "weeks")] %>%
  unique() %>%
  group_by(weeks) %>%
  tally() %>%
...
patient_count_per_week %>%
  ...
  dplyr::mutate(period = ifelse( weeks < pandemic_start_date,
                                  "before_pandemic",
                                  #ifelse(weeks < late_pandemic_start_date,
                                  #       "early_pandemic", 
                                  #       "late_pandemic")
                                 "after_pandemic")) %>% 
...
obs_by_day <- encounters_obs %>% 
  ...
  mutate(calendar_date = as.Date(calendar_date, format=dateFormat), 
         period = if_else(calendar_date < pandemic_start_date,
                          "Before Pandemic",
                          #if_else(calendar_date < late_pandemic_start_date,
                          #        "Early Pandemic", 
                          #        "Late Pandemic")
                          "After Pandemic"
         ), 
...
```{r}
patient_count_per_week_sex <- input_data[, c("patient_num", "sex","weeks")] %>%
  unique() %>%
  group_by(weeks, sex) %>%
  tally() %>%
  mutate(n = ifelse( n > obfuscation | isFALSE(obfuscation),
...
patient_count_per_week_disorder_group <- input_data[input_data$in_hospital == 1,c("patient_num", "weeks", "disorder_group")] %>%
  unique() %>%
...
patient_count_per_week_disorder_group %>%
  mutate(weeks = as.Date(weeks), 
         period = if_else(weeks < pandemic_start_date,
                          "Before Pandemic",
                          #if_else(weeks < late_pandemic_start_date,
                          #        "Early Pandemic", 
                          #        "Late Pandemic")
                          "After Pandemic"
         )
...
summary_race_test <- race_by_period %>% 
  ...
  tidyr::pivot_wider(names_from = period,
                     values_from = count) %>%
  select(after_pandemic) %>% 
  tidyr::replace_na(list(after_pandemic = 0)) %>% 
  chisq.test() %>% 
  `[[`("p.value")
}
```
...
disorder_table1 <- summary_disorder_group_by_period %>% 
  ...
  #                  late_pandemic = "0 (0 %)")) %>% 
    replace_na( list(after_pandemic = "0 (0 %)")) %>% 
...
table1 %>% 
  ...
  ungroup() %>%
  mutate(Category = "") %>%
  mutate(across(.cols = c(before_pandemic, after_pandemic), .fns = replace_na, "0")) %>% 
