...
bootstrapped_sex <- patients_hospitalized_agg_sex_perc_all %>%
  ...
  mutate(month_year = format(time_p, "%m"),
         time = seq_along(time_p))  %>% 
 group_map( ~bootstrapping_slopes(.x, group_name = .y, bootstrap_size = bootstrap_size, count = "count"))
...
quantile_bootstrapped_fitted_sex <- bootstrapped_fitted_sex %>%
  group_by(time_p, period, siteid, sex) %>%
  summarise("2.5%" = quantile(predicted, 0.025),
            "50%" = median(predicted, 0.5),
            "97.5%" = quantile(predicted, 0.975)) %>%
  pivot_longer(cols = c("2.5%", "50%", "97.5%"),
...
```{r}
# expand.grid(
#   unique(df_coefficients_boot_sex$siteid),
#   unique(df_coefficients_boot_sex$sex)
# ) %>% 
#   t() %>% 
#   paste(unique(.$siteid),
#         unique(.$sex), 
...
#   )
# 
# names_plot_annotations_sex <- c()
# for (site in unique(df_coefficients_boot_sex$siteid)) {
#   for (sex in unique(df_coefficients_boot_sex$sex)) {
#     names_plot_annotations_sex <- c(names_plot_annotations_sex, paste(site, sex))
#   }
# }
