...
pvalues_boot_sex <- bootstrapped_coefficients_sex %>% 
  group_by(sex, term) %>% 
  group_modify( bootstrap_pvalue_calculation, .keep = TRUE) %>% 
  mutate(label = paste0("Female proportion period differences: ",
                        observed_slope_coefficient,
                        " (pvalue: ",
                        format.pval(pvalue, digits = 2, eps = 10**-2), ")")
  )
...
ggplot(bootstrapped_fitted_sex,
  ...
  geom_label(data = plot_annotations_sex_df,
             ...
             size = 4,
             inherit.aes = FALSE) +
  annotate("text_female", x = 7, y = 0.85, label = "Female proportion") + 
  annotate("text_male", x = 7, y = 0.15, label = "Male proportion") + 
...
ggsave("figure4_ITS_sex.png", height = 15, width = 20, units = "cm")
```
