...
age_table1_all <- table1_all %>% 
  ...
  group_by(period) %>% 
  summarise(conf_interval = gmodels::ci( mean_age ),
            mean_age = round(sum(Total * mean_age) / sum(Total), 2), 
            sd_age = round(sum(Total * sd_age) / sum(Total), 2), 
            ci_age = paste0("[", round(as.numeric(conf_interval[2]), 2), " , ",  round(as.numeric(conf_interval[3]),2), "]")) %>% 
  mutate(Age = paste0(as.character(mean_age), " (sd: ", as.character(sd_age), ")", as.character(ci_age))) %>% 
  select(period, Age) %>% 
  unique() %>%
...
bootstrap_pvalue_calculation <- function(df, ...) {
  ...
  list("pvalue" = pvalue_coeff_interaction, 
       "observed_diff" = round(unique(df[["observed_diff"]]), 2)
  ) 
}
...
plot_annotations <- df_coefficients_boot %>% 
  group_by(siteid) %>% 
  group_map( bootstrap_pvalue_calculation ) %>% 
...
forest.meta( x = model_meta_siteid) 
...
pvalue_sex_slope_diff <- bootstrap_pvalue_calculation(  df_coefficients_boot_sex[df_coefficients_boot_sex$sex == "female", ]
)

plot_annotations_sex_df <- paste0("Female proportion period differences: ",
                                  pvalue_sex_slope_diff$observed_diff,
                                  " (pvalue: ",
...
```{r bootstrap_plot}
ggplot(bootstrapped_fitted_sex, aes(x = time_p, y = predicted/100, group = interaction(iteration, period, sex))) +
...
bootstrapped_coefficients_disorder <- lapply(bootstrapped_disorders, `[[`, 1) %>% bind_rows()
bootstrapped_fitted_disorder <- lapply(bootstrapped_disorders, `[[`, 2) %>% bind_rows()
