...
median_table1_all <- length_hospitalisation_values_all %>% 
  group_by(period) %>% 
  summarise(median_hospitalisation = paste0(median(len_hospitalisation), " (", IQR(len_hospitalisation), ")")) %>% 
...
pvalues_boot_complete <- pvalues_boot %>% 
  ...
  select(plotsLabel, string) %>% 
  pivot_wider(names_from = term, values_from = string) %>% 
  select(plotsLabel, before_slope, during_slope, difference_slope) %>% 
...
```{r}
#plot_annotations_df_primary <- pvalues_boot %>%
#  select(siteid, label) %>% 
...
bootstrapped_fitted_all %>%
  mutate( time_p = as.Date(time_p)+ 15,
          time_p = as.character(time_p),
                  time_p = if_else(time_p == '2020-04-16', as.character('2020-04-01'), time_p), 
                  time_p = if_else(time_p == '2020-03-16', as.character('2020-03-31'), time_p),
                  time_p = if_else(time_p == '2021-04-16', as.character('2021-04-30'), time_p),
                  time_p = if_else(time_p == '2019-02-16', as.character('2019-02-01'), time_p),
                  time_p = as.Date(time_p))%>%
  ...
  geom_line(aes(color = period),
            alpha = 0.03) +
  geom_line(data = quantile_bootstrapped_fitted_all %>% mutate(time_p = as.Date(time_p)+ 15,  time_p = as.character(time_p),
                  time_p = if_else(time_p == '2020-04-16', as.character('2020-04-01'), time_p), 
                  time_p = if_else(time_p == '2020-03-16', as.character('2020-03-31'), time_p),
                  time_p = if_else(time_p == '2021-04-16', as.character('2021-04-30'), time_p),
                  time_p = if_else(time_p == '2019-02-16', as.character('2019-02-01'), time_p),
  ...
  geom_point(data = patient_count_psy_period_psy_all %>% mutate( time_p = as.Date(time_p)+ 15),
             aes(y = percentage, group=NULL),
             alpha = 0.5) +
  geom_vline(xintercept = as.Date("2020-04-01"), linetype = 6)+
...
```{r}
#plot_annotations_sex_df <- pvalues_boot_sex %>%
#  select(term, sex, label) %>% 
#  filter(term == "difference_slope" & sex == "female") %>% 
#  mutate(plotsLabel = "Female") 

plot_annotations_sex_df <- pvalues_boot_sex_complete %>%
  filter( sex == "female") %>% 
    mutate(plotsLabel = "Female") %>%
  select( label = `Difference before/since pandemic trend [95% CI] (p-value)`, plotsLabel )
...
```{r }
#plot_annotations_sex_df2 <- data.frame(term=c("difference_solpe","difference_solpe"),
#                                       sex=c("female", "male"),
#                                       label=c("Coefficient difference: 0.77 (pvalue: <0.01)", "Coefficient difference: -0.77 (pvalue: 0.005)"),
#                                       plotsLabel=c("Female", "Male"))
...
bootstrapped_fitted_sex %>% 
  ...
  geom_line(aes(color = period),
            alpha = 0.01) +
  geom_line(data = quantile_bootstrapped_fitted_sex[ quantile_bootstrapped_fitted_sex$sex =="female", ] %>% 
              mutate(time_p = as.Date(time_p)+ 15,  time_p = as.character(time_p),
                  time_p = if_else(time_p == '2020-04-16', as.character('2020-04-01'), time_p), 
                  time_p = if_else(time_p == '2020-03-16', as.character('2020-03-31'), time_p),
                  time_p = if_else(time_p == '2021-04-16', as.character('2021-04-30'), time_p),
                  time_p = if_else(time_p == '2019-02-16', as.character('2019-02-01'), time_p),
            ...
            aes(y = value, group = interaction(quantiles, period)),
            linetype = "dashed") +
  geom_line(data = quantile_bootstrapped_fitted_sex[quantile_bootstrapped_fitted_sex$sex =="female" & quantile_bootstrapped_fitted_sex$quantiles == "50%", ]%>% mutate(time_p = as.Date(time_p)+ 15,  time_p = as.character(time_p),
                  time_p = if_else(time_p == '2020-04-16', as.character('2020-04-01'), time_p), 
                  time_p = if_else(time_p == '2020-03-16', as.character('2020-03-31'), time_p),
                  time_p = if_else(time_p == '2021-04-16', as.character('2021-04-30'), time_p),
                  time_p = if_else(time_p == '2019-02-16', as.character('2019-02-01'), time_p),
                  time_p = as.Date(time_p)),
            aes(y = value, group = period)) +
  geom_point(data = patients_hospitalized_agg_sex_total[ patients_hospitalized_agg_sex_total$sex =="female",]  
             %>% mutate( time_p = as.Date(time_p)+ 15),
             aes(y = percentage, group=NULL),
             alpha = 0.5) +
  #scale_colour_manual(values = wesanderson::wes_palette("GrandBudapest1", 3)) +
...
```{r bootstrap_plot, include=FALSE}
plot_sex <- ggplot(bootstrapped_fitted_sex,
  ...
  annotate("label",
           ...
           fill = "#B2DF8A") +
  # annotate("text", x = 7, y = 0.15, label = "Male proportion") +
  labs(
    #title = "Patients sex proportion trends among psychiatric-related visits",
    #   subtitle = "Parametric bootstrap estimation: (10000 iterations)",
       x = "Calendar dates",
       y = "Sex proportion",
       color = "Sex") +
...
bootstrapped_fitted_disorder %>% 
  ...
  geom_line(aes(color = period),
            alpha = 0.03) +
  geom_line(data = quantile_bootstrapped_fitted_disorder %>% mutate(time_p = as.Date(time_p)+ 15,  time_p = as.character(time_p),
                  time_p = if_else(time_p == '2020-04-16', as.character('2020-04-01'), time_p), 
                  time_p = if_else(time_p == '2020-03-16', as.character('2020-03-31'), time_p),
                  time_p = if_else(time_p == '2021-04-16', as.character('2021-04-30'), time_p),
                  time_p = if_else(time_p == '2019-02-16', as.character('2019-02-01'), time_p),
  ...
  geom_line(data = quantile_bootstrapped_fitted_disorder[quantile_bootstrapped_fitted_disorder$quantiles == "50%", ] %>%
              mutate(time_p = as.Date(time_p)+ 15,  time_p = as.character(time_p),
                  ...
                  time_p = as.Date(time_p)),
            aes(y = value, group = period)) +
  geom_point(data = filter(perc_disorder_group_total  %>% mutate( time_p = as.Date(time_p)+ 15),
