...
bootstrapped_fitted_all <-  bootstrapped_fitted_all %>%
  filter( time_p <= end_date_plots &
          time_p >= start_date_plots)

count_icd_all <-  count_icd_all %>%
  filter( time_p <= end_date_plots &
          time_p >= start_date_plots)
...
perc_disorder_group_all <- perc_disorder_group_all %>%
  filter( time_p <= end_date_plots &
          time_p >= start_date_plots)

ratios_patients_with_without_psy_all <- ratios_patients_with_without_psy_all %>%  
  filter( time_p <= end_date_plots &
          time_p >= start_date_plots)
```
...
df_coefficients_boot <- df_coefficients_boot %>%
...
quantile_bootstrapped_fitted_all <- quantile_bootstrapped_fitted_all %>%
  left_join( country_map )

count_icd_all_boot <- count_icd_all_boot %>%
  left_join( country_map )
...
trends_country <- trends_siteid %>% 
  left_join(country_map, by = "plotsLabel") %>% 
  group_by(country) %>% 
