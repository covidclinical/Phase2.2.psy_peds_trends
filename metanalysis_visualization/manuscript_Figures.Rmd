...
for( i in 1:length( outputFiles )){
  ...
  if( i == 1 ){
    ...
    if (outputFiles[i] == "peds_psy_aggResults_CHOP.RData") {
      count_icd_all <- tryCatch(
        rbind(count_icd_all, count_icd),
        error = function(e) count_icd_all
      )
      patients_hospitalized_agg_sex_perc_all <- tryCatch( 
        rbind( patients_hospitalized_agg_sex_perc_all, patients_hospitalized_agg_sex_perc),
        error = function(e) patients_hospitalized_agg_sex_perc_all
      )
      perc_disorder_group_all <- tryCatch(
        rbind(perc_disorder_group_all, perc_disorder_group),
        error = function(e) perc_disorder_group_all
      )
      mental_codes_qc_output_all <- tryCatch(
        rbind( mental_codes_qc_output_all,
               mental_codes_qc_output %>%
                 select( disorder_group, description, concept_code, distinct_patients, distinct_observations, siteid )
        ),
        error = function(e) mental_codes_qc_output_all
      )
...
for( i in 1:length(sites)){
  ...
  p[[i]] <- time_series_analysis_plotting(ts_seasonal_percent_per_site,
                              ...
                              clearance_period = FALSE) +
  labs(title = sites[i],
       subtitle = "ITS with seasonality effect",
       x = "Calendar dates",
       y = "Percentage") +
     theme(strip.text = element_text(size = 10),
        axis.text.x = element_text(size = 8), 
