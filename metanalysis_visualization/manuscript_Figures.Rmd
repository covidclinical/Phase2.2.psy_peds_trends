...
patients_hospitalized_agg_sex_total <- patients_hospitalized_agg_sex_perc_all %>%
  ...
  select(time_p, time, period, sex, count_sex, percentage, siteid)
```
...
perc_disorder_group_complete <- perc_disorder_group_all %>% 
  ...
  group_by(siteid, time_p) %>% # grouping by time prevent following function from wokring as expected
  fill(count_icd, 
       period, 
       country , 
       plotsLabel, 
  ...
  select(-time) %>% 
  # Readfing consistent time information 
  left_join(time_timep_df, by = "time_p")
...
counts <- patient_count_psy_period_psy_all %>%
  select( plotsLabel, time_p, time, count_psy_patients, total_cases = count_icd )

counts2 <- reshape2::melt(counts ,  id.vars = c('plotsLabel', 'time_p', 'time'), variable.name = 'group')
...
counts_agg <- counts %>%
  ...
  select( time_p, time, psy_cases, total_cases)

counts_agg2 <- reshape2::melt(counts_agg ,  id.vars = c('time_p', 'time'), variable.name = 'group')
