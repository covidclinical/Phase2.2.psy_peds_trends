...
listOfPackages <- c("tidyverse", "tidyr","devtools","RColorBrewer", "knitr", "kableExtra", "tsModel", "gridExtra", "dplyr", "metafor", "meta", "viridis", "UpSetR", "scales", "EHRtemporalVariability")
...
patients_hospitalized_agg_sex_perc_all <- patients_hospitalized_agg_sex_perc_all %>% 
  mutate(period = ifelse(time_p <= as.Date("2020-03-15"), "before_pandemic", "during_pandemic")) %>% 
  select(time_p, period, sex, count, percentage, siteid) 

bootstrapped_sex <- patients_hospitalized_agg_sex_perc_all %>%
  group_by(siteid, sex) %>% 
  mutate(month_year = format(time_p, "%m"),
         time = seq_along(time_p))  %>% 
 group_map( ~bootstrapping_slopes(.x, bootstrap_size = bootstrap_size, count = "count"))
names(bootstrapped_sex) <- unique(patients_hospitalized_agg_sex_perc_all$siteid)

bootstrapped_coefficients_sex <- lapply(bootstrapped_sex, `[[`, 1) %>% bind_rows(.id = "siteid")
bootstrapped_fitted_sex <- lapply(bootstrapped_sex, `[[`, 2) %>% bind_rows(.id = "siteid")
...
```{r}
coefficients_df_sex <- patients_hospitalized_agg_sex_perc_all %>% 
  group_by(siteid, sex) %>% 
    mutate(month_year = format(time_p, "%m"),
         time = seq_along(time_p))  %>% 
  group_modify(coefficient_slope_calculation) %>% 
...
df_coefficients_boot_sex <- bootstrapped_coefficients_sex %>% 
  left_join(coefficients_df_sex, by = c("siteid", "sex"))
```


```{r}
names_site_id <- unique(df_coefficients_boot_sex$siteid)

plot_annotations <- df_coefficients_boot_sex %>% 
  group_by(siteid, sex) %>% 
  group_map( annotation_function ) %>% 
  set_names(names_site_id) %>% 
  unlist()
...
counts <- count_icd_all %>%
  mutate( time = seq_along(time_p),
         month_year = format(time_p, "%m")) %>%
  select( siteid, time_p, time, month_year, count_psy_patients = count_psy, total_cases = count_icd )
