...
listOfPackages <- c("tidyverse", "tidyr","devtools","RColorBrewer", "knitr", "kableExtra", "tsModel", "gridExtra", "dplyr", "metafor", "meta", "viridis", "UpSetR", "scales", "EHRtemporalVariability", "plotly", "gmodels")
...
median_table1_all <- length_hospitalisation_values_all %>% 
  group_by(period) %>% 
  summarise(median_hospitalisation = mean(len_hospitalisation)) %>% 
...
mean_table1_all <- length_hospitalisation_values_all %>% 
  ...
  summarise(mean_hospitalisation = round( mean(len_hospitalisation), 2), 
            ...
            ci_length = paste0("[", round(as.numeric(conf_interval[2]), 2), " , ",  round(as.numeric(conf_interval[3]),2), "]")) %>%
...
```{r}
bootstrap_pvalue_calculation <- function(df, slope_name, ...) {
  z <- unique(df[["observed_slope_coefficient"]]) / sd(df[[slope_name]])
  ...
  list("pvalue" = pvalue_coeff_interaction,
...
```{r}
df_coefficients_boot_primary <- bootstrapped_coefficients_all %>% 
  left_join(filter(coefficients_df, term == "periodduring_pandemic:time"),
            by = "siteid") 

names_site_id <- unique(df_coefficients_boot_primary$siteid)

plot_annotations_primary <- df_coefficients_boot_primary %>% 
  group_by(siteid) %>% 
  group_map( bootstrap_pvalue_calculation, slope_name = "difference_slope" ) %>% 
  set_names(names_site_id) %>% 
  lapply(FUN = function(vec) {
    paste0("Coeffs differences: ",
           vec$observed_slope_coefficient,
           " (pvalue: ",
           format.pval(vec$pvalue,
                       digits = 2,
                       eps = 10**-3, 
                       nsmall = 2),
           ")")
...
plot_annotations_df_primary <- data.frame(
  siteid = names(plot_annotations_primary), 
  label = plot_annotations_primary
) %>%
  left_join( country_map ) 
```

```{r}
ggplot(bootstrapped_fitted_all,
       aes(x = time_p, y = predicted, group = interaction(iteration, period))) +
...
ggsave("figure1_boostrapped_all.png", height = 20, width = 45, units = "cm")
```


## Secondary analysis : slopes before/during



```{r}
pvalues_boot_before <- bootstrapped_coefficients_all %>% 
  left_join(filter(coefficients_df, term == "time"), by = "siteid") %>% 
  group_by(siteid) %>% 
  group_map( bootstrap_pvalue_calculation, slope_name = "before_slope" ) %>% 
  setNames(unique(bootstrapped_coefficients_all$siteid)) %>% 
  bind_rows(.id = "siteid") %>% 
  mutate(string = paste0(observed_slope_coefficient, " (", pvalue, ")")) %>% 
  mutate(string = paste0(observed_slope_coefficient, " (", format.pval(pvalue,
                                                                       digits = 2,
                                                                       eps = 10**-3, 
                                                                       nsmall = 2),
                         ")")) %>% 
  select(siteid, string) %>% 
  rename("Before pandemic trend (p-value)" = string)

pvalues_boot_after <- bootstrapped_coefficients_all %>% 
  left_join(coefficients_df %>%
              pivot_wider(names_from = term,
                          values_from = observed_slope_coefficient) %>% 
              mutate(observed_slope_coefficient = time + `periodduring_pandemic:time`) %>% 
              select(siteid, observed_slope_coefficient),
            by = "siteid") %>% 
  group_by(siteid) %>% 
  group_map( bootstrap_pvalue_calculation, slope_name = "during_slope" ) %>% 
  setNames(unique(bootstrapped_coefficients_all$siteid)) %>% 
  bind_rows(.id = "siteid")%>% 
  mutate(string = paste0(observed_slope_coefficient, " (", format.pval(pvalue,
                                                                       digits = 2,
                         ...
                         ")")) %>% 
  select(siteid, string) %>% 
  rename("Since pandemic trend (p-value)" = string)


pvalues_boot_difference <- bootstrapped_coefficients_all %>% 
  left_join(filter(coefficients_df, term == "periodduring_pandemic:time"), by = "siteid") %>% 
  group_by(siteid) %>% 
  group_map( bootstrap_pvalue_calculation, slope_name = "difference_slope" ) %>% 
  setNames(unique(bootstrapped_coefficients_all$siteid)) %>% 
  bind_rows(.id = "siteid") %>% 
  ...
  mutate(string = paste0(observed_slope_coefficient, " (", format.pval(pvalue,
                         ...
                         ")")) %>% 
  select(siteid, string) %>% 
  rename("Before/since pandemic trend difference (p-value)" = string)

inner_join(pvalues_boot_before, pvalues_boot_after) %>% 
  inner_join(pvalues_boot_difference) %>% 
  inner_join(country_map) %>% 
  select(plotsLabel, everything(), -country, -siteid) %>% 
  arrange(plotsLabel) 
# %>% 
#   flextable::flextable() %>% 
#   flextable::autofit() %>% 
#   flextable::save_as_docx(path = "trends_pval.docx")
...
```{r}
trends_siteid <- df_coefficients_boot_primary %>% 
  group_by(siteid) %>% 
  summarise(mean_boot = mean(difference_slope),
            observed = unique(observed_slope_coefficient),
            var_boot = var(difference_slope), 
...
metafor::forest(model_meta_siteid,
                ...
                mlab="Overall effect")
dev.off()
#ggsave("figure2A_metaanalysis_persite.png", height = 20, width = 45, units = "cm")
```
...
coefficients_df_sex <- patients_hospitalized_agg_sex_total %>% 
  ...
  mutate(month_year = format(time_p, "%m"),
         time = seq_along(time_p))  %>% 
  group_modify(coefficient_slope_calculation, 
               slope_name = "periodduring_pandemic:time") %>% 
  rename(observed_slope_coefficient = estimate)
...
plot_annotations_sex <- df_coefficients_boot_sex %>% 
  group_by(sex) %>% 
  group_map( bootstrap_pvalue_calculation, slope_name = "difference_slope" ) %>%
  set_names(names_sex) %>% 
  lapply(FUN = function(vec) {
  ...
  unlist()
plot_annotations_sex_df <- data.frame(
  sex = names(plot_annotations_sex), 
  label = plot_annotations_sex
)
```
...
bootstrapped_fitted_sex %>% 
  ...
  ggplot(aes(x = time_p, y = predicted, group = interaction(iteration, period))) +
  facet_wrap("sex",
             scales = "free_y",
  ...
  theme(strip.text = element_text(size = 10),
        ...
        legend.position = "bottom") 
ggsave("figure4_boostrapped_sex.png", height = 15, width = 25, units = "cm")
```

#### Original plot 

```{r annotation bootstrap disorder sex}
names_sex <- unique(df_coefficients_boot_sex$sex)

## Using only male, but female and male necessarily yield equal p-values

pvalue_sex_slope_diff <- bootstrap_pvalue_calculation(
  df_coefficients_boot_sex[df_coefficients_boot_sex$sex == "female", ], 
  slope_name = "difference_slope"
)

plot_annotations_sex_df <- paste0("Female proportion period differences: ",
                                  pvalue_sex_slope_diff$difference_slope,
                                  " (pvalue: ",
                                  format.pval(pvalue_sex_slope_diff$pvalue,
                                              digits = 2,
                                              eps = 10**-3, 
                                              nsmall = 2),")") %>%
  data.frame(label = ., 
...
```{r bootstrap_plot}
ggplot(bootstrapped_fitted_sex,
  ...
  geom_label(data = plot_annotations_sex_df,
             ...
             size = 4,
             inherit.aes = FALSE) +
  labs(title = "Interrupted Time Series Analysis: Patients sex proportion",
       subtitle = "Parametric bootstrap estimation: (10000 iterations)",
       x = "Calendar dates",
       y = "Sex proportion",
       color = "Sex") +
...
ggsave("figure4_ITS_sex.png", height = 20, width = 20, units = "cm")
```
...
#### Bootstrapping results by disorder groups
...
bootstrapped_coefficients_disorder <- lapply(bootstrapped_disorders, `[[`, 1) %>%
  bind_rows() %>% 
  rename(before_slope = time, 
         difference_slope = `periodduring_pandemic:time`)
...
quantile_bootstrapped_fitted_disorder <- bootstrapped_fitted_disorder %>%
  ...
  group_by(time_p, period,  disorder_group) %>%
  summarise("2.5%" = quantile(predicted, 0.025),
            "50%" = median(predicted, 0.5),
            "97.5%" = quantile(predicted, 0.975)) %>%
  pivot_longer(cols = c("2.5%", "50%", "97.5%"),
               names_to = "quantiles")
coefficients_df_disorder <- perc_disorder_group_total %>% 
  group_by(disorder_group) %>% 
  mutate(month_year = format(time_p, "%m"),
         time = seq_along(time_p))  %>% 
  group_modify(coefficient_slope_calculation, 
...
df_coefficients_boot_disorder <- bootstrapped_coefficients_disorder %>% 
  left_join(coefficients_df_disorder, by = c("disorder_group"))
```

```{r annotation boostrap disorders}
names_disorder <- unique(df_coefficients_boot_disorder$disorder_group)

plot_annotations_disorder <- df_coefficients_boot_disorder %>% 
  group_by(disorder_group) %>% 
  group_map( bootstrap_pvalue_calculation, slope_name = "difference_slope" ) %>%
  set_names(names_disorder) %>% 
  lapply(FUN = function(vec) {
    paste0("Coeffs differences: ",
           vec$observed_slope_coefficient,
           " (pvalue: ",
  ...
  unlist()
plot_annotations_disorder_df <- data.frame(
  disorder_group = names(plot_annotations_disorder), 
  label = plot_annotations_disorder
)
```


```{r}
# plot_annotations_df <- plot_annotations_df %>%
#   left_join( country_map ) 
...
bootstrapped_fitted_disorder %>% 
  ...
  geom_line(aes(color = period),
            alpha = 0.03) +
  geom_line(data = quantile_bootstrapped_fitted_disorder,
            aes(y = value, group = interaction(quantiles, period)),
            linetype = "dashed") +
  geom_line(data = quantile_bootstrapped_fitted_disorder[quantile_bootstrapped_fitted_disorder$quantiles == "50%", ],
            aes(y = value, group = period)) +
  geom_point(data = filter(perc_disorder_group_total,
                           disorder_group %in% disorders_to_select),
             aes(y = percentage, group=NULL),
             alpha = 0.5) +
  scale_colour_manual(values = cbPalette) +
  scale_x_date(breaks = "1 month", labels=date_format("%b-%y"), expand=c(0.03,0))+
...
fig
#plotly::orca(fig, file = "figure_supp3_counts_disorder.png")
```
...
numberOfCodesPerDisorderGroup %>%
  ...
  theme(axis.text.x=element_text(angle=45,hjust=1, size = 6), 
        ...
        panel.grid.major = element_blank(), panel.grid.minor = element_blank(), 
        legend.position="bottom") 
ggsave("figure_supp2_ICDdictionaryQC.png", height = 20, width = 45, units = "cm")
...
counts_plot <- counts2 %>% ggplot( aes( x= as.Date( time_p ), y = value, group= group )) +
  ...
  theme(axis.text.x=element_text(angle = 45, hjust=1, size = 6), 
        ...
        legend.position="bottom")
counts_plot
ggsave("figure_supp1A_counts_per_site.png",counts_plot, height = 20, width = 32, units = "cm")
