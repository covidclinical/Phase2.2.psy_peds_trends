...
### Total number of patients per month an site

```{r}
patients_hospitalized_agg_sex_perc_all %>%
  select( time_p, count, siteid) %>%
  unique() %>%
    filter(time_p <= history_end_date &
         time_p >= history_start_date) %>%
  ggplot(aes(x = time_p, y = count, fill = siteid, color = siteid)) +
  geom_point( aes(shape=siteid, color=siteid)  ) +
  geom_line() +
  geom_vline(xintercept = as.Date(pandemic_start_date),
             linetype = "dashed") +
  ...
  labs(y = "number of patients",
       x = paste0("Date (by ", time_period,")"),
       title = paste0("Total number of patients with mental disorders per site \n ( per ", time_period,")"))
```
...
# }
# 
# # View(patient_count_psy_period_psy_all)
# 
# bootstrap_function <- function(df, ...) {
#   bootstrapped_timepoints <- mapply(binom_proportion,
#          n = bootstrap_size,
#          size = df$count_no_psy_patients +
#            df$count_psy_patients,
...
#   list("bootstrapped_fitted_df" = bootstrapped_fitted_df,
#        "bootstrapped_coefficients_df" = bootstrapped_coefficients_df)
# }
# 
# 
# bootstrapped_all <- patient_count_psy_period_psy_all %>% 
#   group_by(siteid) %>% 
#   group_map(bootstrap_function) %>% 
#   set_names(nm = names_site_id)
# 
# bootstrapped_fitted_all <- purrr::map(bootstrapped_all, "bootstrapped_fitted_df") %>% 
#   bind_rows(.id = "siteid")
# bootstrapped_coefficients_all <- purrr::map(bootstrapped_all, "bootstrapped_coefficients_df") %>% 
#   bind_rows(.id = "siteid")
  ...
  pvalue_coeff_interaction <- round(
    (1- max(
      sum(df[["periodduring_pandemic:time"]] > 0,
          df[["periodduring_pandemic:time"]] < 0)
    )/nrow(df)) * 2
    , 3) 
