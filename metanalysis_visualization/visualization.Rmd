...
annotation_function <- function(df, ...) {
  ...
  z <-  mean(df[["periodduring_pandemic:time"]]) / sd(df[["periodduring_pandemic:time"]])
...
```{r}
# Fixed effects
model_meta_siteid <- metafor::rma(yi = mean_boot, 
             vi = var_boot,
             measure = "MN",
             method = "FE",
  data = trends_siteid) 
...
```{r}
trends_country <- bootstrapped_coefficients_all %>% 
