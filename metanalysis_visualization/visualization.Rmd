...
trends_siteid <- meta_df %>% 
  filter((time_p >= as.Date("2019-02-01")) & (time_p <= as.Date("2021-05-01"))) %>% 
  filter(siteid != "APHP") %>% 
  group_by(siteid) %>% 
...
### UpSet R for each disorder group
```{r}
disorders <- unique( mental_codes_qc_output_all$disorder_group)
for( i in 1:length(disorders) ){
  print( disorders[i] )
  upsetForDisorderGroup( df = mental_codes_qc_output_all, disorderGroup = disorders[i])
}
```
